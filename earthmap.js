...
const NATURAL_EARTH_ARCHIVE_DATASETS = [
  {
    ...
    detail: "10m",
    path: "../assets/geojson/natural-earth/10m/ne_10m_admin_0_countries.geojson",
  ...
  },
  {
    ...
    detail: "10m",
    path: "../assets/geojson/natural-earth/10m/ne_10m_admin_1_states_provinces.geojson",
    status: "GeoJSON eingebunden",
  },
  {
    id: "lakes",
...
async function loadNaturalEarthAdmin1Dataset() {
  ...
  renderProjectBrowser();
  try {
    const response = await fetch("../assets/geojson/natural-earth/10m/ne_10m_admin_1_states_provinces.geojson", { cache: "force-cache" });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const data = await response.json();
    state.naturalEarthAdmin1Dataset = {
      detail: "10m",
      label: "10m · Natural-Earth-Admin-1",
      sourceUrl: "../assets/geojson/natural-earth/10m/ne_10m_admin_1_states_provinces.geojson",
...
function requestNaturalEarthAdmin1BoundaryLayerForZoom() {
  ...
  script.src = "../assets/geojson/natural-earth/10m/ne_10m_admin_1_states_provinces.boundaries.js?v=20260709a";
  script.async = true;
  script.onload = () => {
...
function getNaturalEarthAdmin1BoundaryRings() {
  ...
  state.naturalEarthAdmin1BoundaryRings = extractLandPolygons(source).flat();
...
function drawNaturalEarthLakeLayer(radius, center) {
  ...
  const stroke = document.body.classList.contains("earthmap-theme-dark")
    ? "rgba(255,238,183,.16)"
    : "rgba(92,96,94,.22)";
  const lineWidth = getStableGlobeStrokeWidth(radius, 0.001, 0.36);
  rings.forEach((ring) => {
    if (!ring?.length) return;
    drawProjectedLine(densifyRing(ring, density), radius, center, stroke, lineWidth);
  });
  return true;
}
...
function drawNaturalEarthAdmin0BoundaryLayer(radius, center) {
  ...
  const stroke = document.body.classList.contains("earthmap-theme-dark")
    ? `rgba(255,238,183,${Math.min(0.5, alpha + 0.04)})`
    : `rgba(66,72,70,${alpha})`;
...
function drawNaturalEarthAdmin1BoundaryLayer(radius, center) {
  ...
  const stroke = document.body.classList.contains("earthmap-theme-dark")
    ? `rgba(255,238,183,${Math.min(0.48, alpha)})`
    : `rgba(82,88,86,${alpha})`;
...
function renderGlobe() {
  ...
  if (shouldRenderContinentalBaseMap()) {
    ...
    drawNaturalEarthAdmin0BoundaryLayer(radius, center);
  }
  drawNaturalEarthAdmin1BoundaryLayer(radius, center);
  if (state.showGraticule) drawGraticule(radius, center);
  drawProjectBoundaryLayers(radius, center);
    ...
    .map(([title, items]) => ({
      ...
      items: items
        .map((feature) => {
          const properties = feature.properties || {};
          return {
            name: repairLegacyText(properties.NAME_EN || properties.NAME || properties.ADMIN || "Unbenannt"),
            type: repairLegacyText(properties.TYPE || "Einheit"),
            iso3: properties.ADM0_A3 || properties.ISO_A3 || "",
    ...
    .map(([title, items]) => ({
      ...
      items: items
        .map((feature) => {
          const properties = feature.properties || {};
          return {
            name: repairLegacyText(properties.name_de || properties.name || properties.name_en || properties.adm1_code || "Unbenannt"),
            type: repairLegacyText(properties.type || properties.type_en || "Gliedstaat / Provinz"),
            iso3: properties.adm0_a3 || properties.sov_a3 || "",
