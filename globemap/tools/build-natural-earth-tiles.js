...
const sourcePath = path.join(root, "data", "natural-earth", "10m", "ne_10m_land.geojson");
const outputDir = path.join(root, "data", "natural-earth", "10m", "tiles");
const tileStep = 30;
...
for (const tile of nonEmptyTiles) {
  ...
  const js = [
    ...
    `window.GlobeMapNaturalEarthTileData['10m-land-${tile.key}'] = ${JSON.stringify(collection)};`,
    "",
  ].join("\n");
...
fs.writeFileSync(
  ...
  [
    "window.GlobeMapNaturalEarthTileIndex = window.GlobeMapNaturalEarthTileIndex || {};",
    `window.GlobeMapNaturalEarthTileIndex['10m-land'] = ${JSON.stringify(index)};`,
    "",
  ].join("\n"),
  "utf8",
);
